...
int model_load(CnnModel *m, const char *path) {
    ...
    err |= read_floats(f, m->conv1_b,
                       sizeof m->conv1_b / sizeof m->conv1_b[0], "conv1_b");
    ...
    err |= read_floats(f, m->conv3_b,
                       sizeof m->conv3_b / sizeof m->conv3_b[0], "conv3_b");
    err |= read_floats(f, m->conv4_w,
                       sizeof m->conv4_w / sizeof m->conv4_w[0], "conv4_w");
    err |= read_floats(f, m->conv4_b,
                       sizeof m->conv4_b / sizeof m->conv4_b[0], "conv4_b");
    err |= read_floats(f, m->fc_w,
                       sizeof m->fc_w / sizeof m->fc_w[0], "fc_w");
    err |= read_floats(f, m->fc_b,
                       sizeof m->fc_b / sizeof m->fc_b[0], "fc_b");
